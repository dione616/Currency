...
const Converter: React.FC<Props> = (props) => {
  ...
  const handleConvert = () => {
    if (!reversed2.done) {
      const searchCurrency = mapedData.filter((note) => {
        if (
          note.base_ccy === leftCurrency.currency &&
          note.ccy === rightCurrency.currency
        )
          return note;
        else if (
          note.ccy === leftCurrency.currency ||
          note.ccy === rightCurrency.currency
        ) {
          return note;
        }
      ...
      if (searchCurrency.length > 1) {
        convertedValue =
          (leftCurrency.count * parseFloat(searchCurrency[1].buy)) /
        ...
        convertedValue =
          leftCurrency.count / parseFloat(searchCurrency[0].sale);
      ...
      const searchCurrency = mapedData.filter((note) => {
        return (
          note.ccy === leftCurrency.currency &&
          note.base_ccy === rightCurrency.currency
        );
      });
      ...
      if (searchCurrency.length > 0) {
        ...
        if (reversed2.firsTime) {
          ...
          convertedValue =
            rightCurrency.count / parseFloat(searchCurrency[0].sale);
        }
  ...
  useEffect(() => {
    if (!reversed2.done) {
      if (
        handleConvert().count !== rightCurrency.count ||
        handleConvert().currency !== rightCurrency.currency
      ) {
    ...
    <div className="wrapper">
      <div className="box">
        ...
        <div className="middle">
          ...
          {!converted ? (
            <div className="error">Wrong pair to convert</div>
          ) : null}
        </div>
      ...
      <p className="note">
        Note: converting USD to EUR will first convert USD to UAH and then to
        EUR. Wich will cost additioonal money
      </p>
    </div>
  );
};
